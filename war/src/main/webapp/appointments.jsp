        ...
        <div class="flex items-center justify-center gap-4 mt-5">
          ...
          <c:choose>
            <c:when test="${currentPage > 1}">
              <a
                href="appointments?page=${currentPage - 1}"
          ...
          <div class="flex items-center gap-2">
            <c:forEach begin="${startPage}" end="${endPage}" var="i">
              <a
                href="${pageContext.request.contextPath}/appointments/view/${i}${not empty param.search ? '?search=' + fn:escapeXml(param.search) : ''}"
                class="relative h-10 w-10 ... ${currentPage == i ? 'bg-gray-900 text-white' : 'text-gray-900'}"
              >
                <span
